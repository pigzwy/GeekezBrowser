...
```json
{
  ...
  "message": null,
  "debugPort": 37957,
  "wsEndpoint": "ws://127.0.0.1:37957/devtools/browser/a2ac635a-..."
}
```
...
**响应（成功关闭）**:
```json
{"ok": true, "profileId": "...", "message": "Stopped"}
```

**响应（环境未运行）**:
```json
{"ok": true, "profileId": "...", "message": "Not running"}
```
...
ws_endpoint = resp.json()["wsEndpoint"]
...
| 启动环境 | 返回 CDP | 不返回 CDP |
| 关闭环境 | `POST /profiles/:id/stop` | `POST /api/profiles/:id/stop` |
